...
async fn main() -> Result<()> {
    ...
    match &cli.command {
        ...
        Commands::Generate { config_type, output } => {
...
fn check_config(config_path: &str) -> Result<()> {
    ...
    match AppConfig::load_server_config(config_path) {
        Ok(server_config) => {
            ...
            if !server_config.cert_path.exists() {
                println!("⚠ Warning: Certificate file not found: {:?}", server_config.cert_path);
            } else {
            ...
            if !server_config.key_path.exists() {
                println!("⚠ Warning: Key file not found: {:?}", server_config.key_path);
            } else {
    ...
    match AppConfig::load_client_config(config_path) {
        Ok(client_config) => {
            ...
            println!("✓ Auth key: {} characters", client_config.client.auth_key.len());
            ...
            if client_config.proxies.is_empty() {
                ...
                for (idx, proxy) in client_config.proxies.iter().enumerate() {
                    println!("  Proxy #{}: '{}' (publish_port={}, local_port={})", 
                        idx + 1, proxy.name, proxy.publish_port, proxy.local_port);
                }
            }
